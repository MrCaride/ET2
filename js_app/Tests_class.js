class test{
    ...
    test_entidad() {
        ...
        let formContainer = document.getElementById('IU_form');
        if (!formContainer) {
            formContainer = document.createElement('div');
            formContainer.id = 'IU_form';
            document.body.appendChild(formContainer);
        }
    
        // Load the form before running tests
        this.cargar_formulario_html();
    
        // Add error handling for test execution
        try {
            // Original test code
            ...
            for (let i = 0; i < this.array_pruebas.length; i++) {
                try {
                    ...
                    // Set field value
                    const element = document.getElementById(campotest);
                    if (element) {
                        element.value = valortest;
                    } else {
                        console.error(`Test field ${campotest} not found in DOM`);
                        continue;
                    }
                    ...
                    // Compare results
                    var resultadoestetest = respuestatest === resultadotest ? 'CORRECTO' : 'INCORRECTO';
                    ...
                    console.error(`Error in test ${i}:`, testError);
                    continue;
                }
            }
